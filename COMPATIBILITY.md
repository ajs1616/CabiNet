...
> 1. **G2S machine, any brand (IGT included):** set the host URL on the machine to `http://192.168.50.2:8081/G2S` — a one-time per-machine step (IGT AVP screens in `deploy/AVP_SETUP.md`) — and if the machine offers a G2S **flavor/dialect selector, pick IGT**. *(Automatic host delivery over DHCP is a work in progress; for now every brand takes the host URL by hand.)*
...
| Alpha 2 Pro V32 / Pro Wave / Pro Curve | Direct G2S in supported builds — **Pro Curve bench-proven** (see Bench-test log) | Modern Alpha 2 deployments with current OS. Bench 2026-09: joins direct-G2S; TITO ticket-out and ticket-in work; WAT (AFT) transfer **out** of the machine works; WAT transfer **in** is refused by the cabinet without a physical card reader. |
...
| 2026-09-14..17 | Bally Alpha 2 Pro Curve (`BAL_000000000109`, Bally "Alpha-II") | direct G2S, host URL set in the operator menu; watProfile interfaceMode=hostControl, cashOutMode=anyDevice | **Direct G2S LIVE**: joins and stays up (the bench unit's clock ran 70 s ahead of the hub and read every host request as expired until the hub started stretching timeToLive by the measured lead — v0.3.1); voucher class live — **ticket-OUT issued and ticket-IN redeemed** through the hub (VCE103/VCE108); WAT class live — **transfer OUT (fromEgm) commits** (€29,737.70 cashed out to a player wallet, WTE107 committed) | **WAT transfer IN (toEgm) is refused by the cabinet**: every host-initiated transfer runs requestPending → initiateTransfer → commitTransfer and ends `egmException=99` (transfer aborted, reason unknown), on both the house and a player account, with the machine's own AFT-to-game option enabled. The cabinet expects a physical player card in its reader for money going onto the machine; without one the transfer-in path is unavailable. Fund the machine with a ticket instead. Also seen: `wat/2` answers APX003 (Invalid Device Identifier) — only device 1 exists — and a host-initiated request on the wrong device draws APX012 (Command Restricted To Owner and Guests). |
